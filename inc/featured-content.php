...
class Glades_Featured_Content {
	 ...
	 *
	 * @uses Glades_Featured_Content::$max_posts
	 */
	public static function init() {

		add_filter( 'glades_get_featured_content',     	  array( __CLASS__, 'get_featured_posts' )    );
	 ...
	 * Get featured posts
	 *
	 * @uses Glades_Featured_Content::get_featured_post_ids()
	 *
	 * @return array
	 */
	...
	public static function get_featured_post_ids() {
		...
		if ( ! empty( $featured_ids ) ) {
			return array_map( 'absint', apply_filters( 'glades_featured_content_post_ids', (array) $featured_ids ) );
		}
		...
		if ( $term ) {
			...
			return apply_filters( 'glades_featured_content_post_ids', array() );
		}
		...
		return apply_filters( 'glades_featured_content_post_ids', $featured_ids );
	}
	 ...
	 * @see Glades_Featured_Content::validate_settings().
	 */
	 ...
	 *
	 * @uses Glades_Featured_Content::get_featured_post_ids();
	 * @uses Glades_Featured_Content::get_setting();
	 ...
	 * is deleted by Glades_Featured_Content::validate_settings().
	 *
	 ...
	 *
	 * @uses Glades_Featured_Content::validate_settings()
	 *
	 * @return void
	 */
	...
	public static function customize_register( $wp_customize ) {
		$wp_customize->add_section( 'glades_featured_content', array(
			'title'          => __( 'Featured Content', 'glades' ),
			'description'    => sprintf( __( 'Easily feature all posts with the <a href="%1$s">"featured" tag</a> or a tag of your choice. Your theme supports up to %2$s posts in its featured content area.', 'glades' ), admin_url( '/edit.php?tag=featured' ), absint( self::$max_posts ) ),
			'priority'       => 40,
		 ...
		 *
		 * Sanitization callback registered in Glades_Featured_Content::validate_settings().
		...
		$wp_customize->add_control( 'featured-content[tag-name]', array(
			'label'          => __( 'Tag name', 'glades' ),
			'section'        => 'glades_featured_content',
			'priority'       => 20,
		) );
		$wp_customize->add_control( 'featured-content[hide-tag]', array(
			'label'          => __( 'Hide tag from displaying in post meta and tag clouds.', 'glades' ),
			'section'        => 'glades_featured_content',
			'type'           => 'checkbox',
			'priority'       => 30,
		) );
		$wp_customize->add_control( 'featured-content[show-all]', array(
			'label'          => __( 'Display featured posts in latest blog post listing.', 'glades' ),
			'section'        => 'glades_featured_content',
			'type'           => 'checkbox',
			'priority'       => 40,
		) );
	...
	public static function enqueue_scripts() {
		wp_enqueue_script( 'glades-featured-content-suggest', get_template_directory_uri() . '/js/featured-content-admin.js', array( 'jquery', 'suggest' ), '20150401', true );
	}
	 ...
	 * Glades_Featured_Content::get_featured_content().
	 *
	 * @uses Glades_Featured_Content::delete_transient()
	 *
	 * @param array $input
	 * @return array $output
	 */
