...
add_action('wp_enqueue_scripts', 'cardigan_enqueue_scripts');
...
function cardigan_enqueue_scripts() {
	...
	// Register Genericons
	wp_enqueue_style('cardigan-genericons', get_template_directory_uri() . '/css/genericons/genericons.css');
...
add_action( 'comment_form_before', 'cardigan_enqueue_comment_reply' );

function cardigan_enqueue_comment_reply() {
...
add_action( 'after_setup_theme', 'cardigan_setup' );

if ( ! function_exists( 'cardigan_setup' ) ):
function cardigan_setup() {
	...
	// Add Theme Support for Cardigan Pro Plugin
	add_theme_support( 'cardigan-pro' );

	// Register Navigation Menus
	register_nav_menu( 'primary', __('Main Navigation', 'cardigan') );
	register_nav_menu( 'secondary', __('Top Navigation', 'cardigan') );
	register_nav_menu( 'footer', __('Footer Navigation', 'cardigan') );
	
	// Register Social Icons Menu
	register_nav_menu( 'social', __('Social Icons', 'cardigan') );
...
add_action( 'after_setup_theme', 'cardigan_add_image_sizes' );
...
// Register Sidebars
add_action( 'widgets_init', 'cardigan_register_sidebars' );

if ( ! function_exists( 'cardigan_register_sidebars' ) ):
function cardigan_register_sidebars() {

	// Register Sidebars
	register_sidebar( array(
		'name' => __( 'Sidebar', 'cardigan' ),
		'id' => 'sidebar',
		'description' => __( 'Appears on posts and pages except front page and fullwidth template.', 'cardigan' ),
	...
	register_sidebar( array(
		'name' => __( 'Magazine Homepage', 'cardigan' ),
		'id' => 'magazine-homepage',
		'description' => __( 'Appears on Magazine Homepage template only. You can use the Category Posts widgets here.', 'cardigan' ),
...
add_filter( 'wp_title', 'cardigan_wp_title', 10, 2 );

function cardigan_wp_title( $title, $sep = '' ) {
	global $paged, $page;
	...
	if ( $paged >= 2 || $page >= 2 )
		$title = "$title $sep " . sprintf( __( 'Page %s', 'cardigan' ), max( $paged, $page ) );
...
function cardigan_default_menu() {
...
add_filter('excerpt_length', 'cardigan_excerpt_length');
function cardigan_excerpt_length($length) {
    return 60;
}


// Slideshow Excerpt Length
function cardigan_slideshow_excerpt_length($length) {
    return 32;
}

// Category Posts Large Excerpt Length
function cardigan_category_posts_large_excerpt($length) {
    return 32;
}

// Category Posts Medium Excerpt Length
function cardigan_category_posts_medium_excerpt($length) {
    return 20;
}
...
if ( ! function_exists( 'cardigan_list_comments' ) ):
function cardigan_list_comments($comment, $args, $depth) {
	...
	if( $comment->comment_type == 'pingback' or $comment->comment_type == 'trackback' ) : ?>
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			<p><?php _e( 'Pingback:', 'cardigan' ); ?> <?php comment_author_link(); ?>
			<?php edit_comment_link( __( '(Edit)', 'cardigan' ), '<span class="edit-link">', '</span>' ); ?>
			</p>
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			...
			<div id="div-comment-<?php comment_ID(); ?>" class="comment-body">
				...
				<div class="comment-author vcard">
					...
					<?php printf(__('<span class="fn">%s</span>', 'cardigan'), get_comment_author_link()) ?>
				</div>

		<?php if ($comment->comment_approved == '0') : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'cardigan' ); ?></p>
		<?php endif; ?>

				<div class="comment-meta commentmetadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>"><?php printf(__('%1$s at %2$s', 'cardigan'), get_comment_date(),  get_comment_time()) ?></a>
					<?php edit_comment_link(__('(Edit)', 'cardigan'),'  ','') ?>
				</div>
